...
counting=True
class MatrixSolve:
    def __init__(self, n, m, P, v=0,use_tree=False):
    ...
    def get_addition(self, I, without=None):
        ...
        for i in A:
            if i not in I:
                if (without is None) or (i not in without):
                    I_add.append(i)
        return I_add
    ...
    def solve(self, I=None, best=None, first=-1, last=-1, forced_down=False):
        ...
        count+=1
        if count%100==0:
            if self.v or counting:
                print(f'count={count/factorial(self.n):.16f}%\t{count}')
        if I is None:
        ...
        for i in I_add:
            ...
            if self.use_tree:self.tree.create_node(f"nu={nu_i[i]}─({i+1})",' '.join(map(str, I + [i])), parent=' '.join(map(str, I)))
...
def main(rR, c, n, m, forced_down=False, first=-1, last=-1, lin=True, v=0,use_tree=True):
    global count
    count=0
    start = time.time()
    R = []
...
if __name__ == "__main__":
    ...
    s, a,solv, t=main(r,c,n,m,use_tree=True)
    print(s)
    print(t)
    input("next?")
    print("ok")
    s, a, solver, t = main(r, c, n, m, forced_down=True,use_tree=True)
    print(s)
    print(t)
    input("next?")
    print("ok")
    ...
    for i in range(n):
        print(f'last={i+1}')
        ans_str, a, solver,t=main(r,c,n,m,last=i)
        print(ans_str)
        ans.append(a)
        input("next?")
        print("ok")
    ...
    for i in range(n):
        print(f'first={i + 1}')
        ans_str, a, solver, t = main(r, c, n, m, first=i)
        print(ans_str)
        ans.append(a)
    ...
    for i in range(n):
        for j in range(n):
            ...
            ans_str, a, solver, t=main(r,c,n,m,first=i,last=j)
            print(ans_str)
            rr[i][j]=a[0]
            ans[(i,j)]=a
