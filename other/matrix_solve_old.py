...
counting=True
class MatrixSolve:
    def __init__(self, n, m, P, use_tree=True, v=0):
    ...
    def solve(self, I, best=None, first=-1, last=-1, forced_down=False):
        ...
        if I is not None: # новое решение
        ...
        if forced_down:  # предварительное решение
            self.get_first()
            if self.v: print(self.first_I[:-1])
            nu = self.get_nu(self.first_I[:-1])
            if self.v: print(nu)
            best = [nu, []]
            forced_down = False
        ...
        if len(best) and best[0] < nu_i_sort[0][1]: # наименьшая из веток плоха => отсекаем все ветки
            return best
